...
void s3c_i2c0_cfg_gpio(struct platform_device *dev)
{
	...
	s3c_gpio_pullup(S3C_GPB5, 2);	//bxl@hhtech
	      ...
	      char **cmdline, struct meminfo *mi)
{
	...
	mi->bank[0].size = 91*1024*1024;
#elif defined(CONFIG_RESERVED_MEM_TV_MFC_POST_JPEG_CAMERA)  //bxl@hhtech
	mi->bank[0].size = 83*1024*1024;
/*
...
void hsmmc_set_gpio (uint channel, uint width)
{
	...
	case 2:
		/* GPIO H : Command, Clock */
		#if 1     //bxl@hhtech, reset 88w8686
		gpio_direction_output(S3C_GPN11, 0);
		mdelay(2);
		gpio_direction_output(S3C_GPN11, 1);
		#endif
...
struct s3c_hsmmc_cfg s3c_hsmmc0_platform = {
	...
	.host_caps = HOST_CAPS,
	.bus_width = 4,
	/* Not set to 1 in case some SD card! bxl@hhtech*/
